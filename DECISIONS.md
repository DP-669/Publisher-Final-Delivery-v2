...
- Automatic fallback: a 400 INVALID_ARGUMENT whose message mentions "schema" re-issues the same request in prompt mode, marks the track `call_a_mode="prompt-fallback"`, logs a warning and shows a sidebar warning; any other error raises · Damir: never silently downgrade · `engine._call_a`. Caveat: the 2026-09-14 400s read only "Request contains an invalid argument." (no "schema"), so a rejection worded that way surfaces as an analysis error, not a fallback.
